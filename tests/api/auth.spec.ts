...
test.describe('认证 API', () => {
  ...
  test.describe('[P0] POST /api/v1/auth/register', () => {
    test('[P0] 应该成功注册新用户', async ({ api }) => {
      // WHEN: 发送注册请求
      const response = await api.post(`${API_URL}/api/v1/auth/register`, {
        data: {
    ...
    test('[P1] 重复邮箱应返回 409 Conflict', async ({ api }) => {
      // GIVEN: 先注册一个用户
      await api.post(`${API_URL}/api/v1/auth/register`, {
        data: testUser,
      });
    ...
    test('[P1] 无效邮箱格式应返回 400 Bad Request', async ({ api }) => {
      // WHEN: 发送无效邮箱的注册请求
      const response = await api.post(`${API_URL}/api/v1/auth/register`, {
        data: {
          email: 'not-an-email',
    ...
    test('[P1] 密码过短应返回 400 Bad Request', async ({ api }) => {
      // WHEN: 发送密码过短的注册请求
      const response = await api.post(`${API_URL}/api/v1/auth/register`, {
        data: {
          email: testUser.email,
  ...
  test.describe('[P0] POST /api/v1/auth/login', () => {
    ...
    test('[P1] 错误密码应返回 401 Unauthorized', async ({ api }) => {
      // WHEN: 使用错误密码登录
      const response = await api.post(`${API_URL}/api/v1/auth/login`, {
        data: {
          email: testUser.email,
    ...
    test('[P1] 不存在的用户应返回 401 Unauthorized', async ({ api }) => {
      // WHEN: 使用不存在的用户登录
      const response = await api.post(`${API_URL}/api/v1/auth/login`, {
        data: {
    ...
    test('[P1] 缺少字段应返回 400 Bad Request', async ({ api }) => {
      // WHEN: 缺少密码字段
      const response = await api.post(`${API_URL}/api/v1/auth/login`, {
        data: {
          email: testUser.email,
  ...
  test.describe('[P1] POST /api/v1/auth/refresh', () => {
    ...
    test('[P1] 应该使用 refresh token 获取新的 access token', async ({ api }) => {
      // WHEN: 使用 refresh token 刷新
      const response = await api.post(`${API_URL}/api/v1/auth/refresh`, {
        headers: {
    ...
    test('[P1] 无效的 refresh token 应返回 401', async ({ api }) => {
  ...
  test.describe('[P1] POST /api/v1/auth/logout', () => {
    ...
    test('[P1] 应该成功登出已登录用户', async ({ api }) => {
      // WHEN: 登出
      const response = await api.post(`${API_URL}/api/v1/auth/logout`, {
        headers: {
    ...
    test('[P1] 未登录用户登出应返回 401', async ({ api }) => {
      // WHEN: 未登录时尝试登出
      const response = await api.post(`${API_URL}/api/v1/auth/logout`);
